...
export default function SkillsSection({
  ...
  const { horizontalFlow = false } = design;
  ...
  const containerWrapperStyle = horizontalFlow ? {
    display: 'flex',
    flexWrap: 'wrap',
    gap: '0.25rem 0.75rem',
  } : {};
    ...
    <div>
      ...
      <div style={containerWrapperStyle}>
        {renderIndices.map((idx) => {
          ...
          const displayContent = (skill || '').trim() === '' ? 'Add skill' : skill;
          
          const itemStyle = {
            ...sectionStyle,
            position: 'relative',
            background: horizontalFlow ? 'transparent' : 'transparent',
            padding: horizontalFlow ? '0.1rem 0.5rem' : '0',
            borderRadius: horizontalFlow ? '0.25rem' : '0',
            marginBottom: horizontalFlow ? '0' : '0.2rem',
          };
            ...
            <div key={idx} style={itemStyle}>
              ...
              {isFocused ? (
                <textarea
                  ...
                  onBlur={handleBlur}
                  placeholder="Skill"
                  style={{ width: horizontalFlow ? 'auto' : '100%', minWidth: '80px', border: 'none', resize: 'none', overflow: 'hidden', background: 'transparent', outline: 'none', fontSize: `${(0.6 + offset).toFixed(3)}rem`, boxSizing: 'border-box' }}
                ...
                <div
                  ...
                  style={{ width: horizontalFlow ? 'auto' : '100%', whiteSpace: 'pre-wrap', wordWrap: 'break-word', fontSize: `${(0.6 + offset).toFixed(3)}rem`, padding: '0.2rem', minHeight: '1.5rem', color: displayContent === 'Add skill' ? '#a0a0a0' : 'inherit' }}
